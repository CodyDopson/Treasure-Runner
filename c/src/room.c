...
static const Switch *room_find_switch(const Room *r, int required_switch_id){
    if (r == NULL || r->switches == NULL || r->switch_count <= 0 || required_switch_id < 0){
        return NULL;
    }

    if (required_switch_id < r->switch_count){
        return &r->switches[required_switch_id];
    }
...
static bool room_switch_portal_unlocked(const Room *r, const Switch *sw){
    if (r == NULL || sw == NULL || r->portals == NULL || r->portal_count <= 0){
        return false;
    }

    if (sw->portal_id >= 0 && sw->portal_id < r->portal_count){
        return !r->portals[sw->portal_id].gated;
    }

    for (int i = 0; i < r->portal_count; ++i){
        if (r->portals[i].id == sw->portal_id){
            return !r->portals[i].gated;
        }
    }
...
static bool room_has_pushable_on_switch(const Room *r, const Switch *sw){
    if (r == NULL || sw == NULL || r->pushables == NULL || r->pushable_count <= 0){
        return false;
    }
...
static bool room_switch_is_pressed(const Room *r, int required_switch_id){
    const Switch *sw = room_find_switch(r, required_switch_id);
    if (sw == NULL){
        return false;
    }

    if (room_switch_portal_unlocked(r, sw)){
        return true;
    }

    return room_has_pushable_on_switch(r, sw);
}
