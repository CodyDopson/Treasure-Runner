...
/* Helper: check whether all world treasures are currently collected */
static bool completion_reached(const GameEngine *eng){
    if (eng == NULL || eng->player == NULL || eng->graph == NULL){
        return false;
    }

    int total_treasures = compute_total_treasure_count(eng->graph);
...
Status game_engine_get_total_treasure_count(const GameEngine *eng, int *count_out){
    ...
    *count_out = compute_total_treasure_count(eng->graph);
    return OK;
}
...
Status game_engine_is_game_over(const GameEngine *eng, bool *is_over_out){
    ...
    *is_over_out = completion_reached(eng);
    return OK;
}
...
Status game_engine_is_victory(const GameEngine *eng, bool *is_victory_out){
    ...
    *is_victory_out = completion_reached(eng);
    return OK;
}
